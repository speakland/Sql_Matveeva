...
CREATE TABLE IF NOT EXISTS public.Trainer_team
(
    Trainer_team_id    integer,
    Team_id            integer NOT NULL,
    Management_surname text    NOT NULL
);
...
(
    Trainer_id      integer NOT NULL,
    Trainer_team_id integer NOT NULL,
    Start_date      date    NOT NULL,
    End_date        date
);
...
ALTER TABLE IF EXISTS public.Trainer_team_date
    ADD CONSTRAINT fk_trainer_team
        FOREIGN KEY (Trainer_team_id)
            REFERENCES public.Trainer_team (Trainer_team_id)
            ON UPDATE NO ACTION
            ON DELETE CASCADE;
